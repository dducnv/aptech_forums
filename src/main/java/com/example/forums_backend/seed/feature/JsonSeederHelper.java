...
//        System.out.println(gson.toJson(products));
    ...
    @Override
    public void readManuallyFromResources1() throws IOException {
        File file = ResourceUtils.getFile(DEFAULT_PATH + FILE_PRODUCT);
        FileInputStream fileInputStream = new FileInputStream(file);
        Workbook workbook = new XSSFWorkbook(fileInputStream);
        System.out.println("Feel like reading file is ok.");
        // Có thể đọc nhiều sheet và save vào nhiều bảng.
        ...
        for (Row row : sheet) {
            ...
            System.out.println(jsonData);
            try {
                obj = gson.fromJson(jsonData, Product.class);
                products.add(obj);
            } catch (Exception ex) {
                System.err.println("Can't parse information from json.\nError: " + ex.getMessage());
            }
